...
$sid = $_GET['sid'];
$msg = "";
$sql = "SELECT * FROM `tbl_student` WHERE `stud_id` LIKE '$sid' ";
$rlt = dbset($sql, 3);

if (isset($_POST['submit'])) {
    $status = $_POST['status'];

    $sql2 = "UPDATE `tbl_student` SET `sts` = '$status' WHERE `stud_id` LIKE '$sid' ";
    $rlt2 = dbset($sql2, 1);
    $msg = "Update Success";
}else {
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <h5>Status: <span class="text-primary"><?php if ($rlt[6] == 0) { ?>Active <?php } else { ?>Inactive <?php } ?></span></h5>
                            </div>
                            <div class="col-md-4">
