...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT *  FROM `tbl_student` WHERE `email` LIKE '$email' AND `phone` LIKE '$phnumb' ";
    $rlt = dbset($sql, 3);
    if ($rlt != 0) {
        $sql = "UPDATE `tbl_student` SET `pass`='$newpass' WHERE `email` LIKE '$email' AND `phone` LIKE '$phnumb' ";
        dbset($sql, 1);
